...
class SearchResultsPage extends StatefulWidget {
  ...
  const SearchResultsPage({Key key, this.city}) : super(key: key);
...
class _SearchResultsPageState extends State<SearchResultsPage> {
  ...
  void _onRefresh() async {
    _future =
        ApiController.dailyForecast(days: 1, location_key: widget.city.key);
    _5DayFuture =
        ApiController.dailyForecast(days: 5, location_key: widget.city.key);
    setState(() {});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.city.englishName),
      ),
      body: SmartRefresher(
                            ...
                            children: [
                              ...
                              Text(
                                '${snapshot.data.dailyForecasts.first.temperature.minimum.value.toStringAsFixed(0)} ' +
                                    snapshot.data.dailyForecasts.first
                              ...
                              Text(
                                snapshot
                                    .data.dailyForecasts.first.day.iconPhrase,
                                style: TextStyle(
                                  fontSize: 16.0,
                                  color: kDarkBlue.withOpacity(0.8),
                                ),
